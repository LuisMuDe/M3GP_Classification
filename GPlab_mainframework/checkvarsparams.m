...
%
%   Copyright (C) 2003-2009 Sara Silva (sara@dei.uc.pt)
...
if start 
   ...
   %(this line has been wrong ever since version 3.0 came out!)
   if params.fixedlevel && isempty(params.realmaxlevel)
      if strcmp(params.depthnodes,'1') %depth
   ...
   % workaround for the "keepevals" bug:
   % (when using a test set, don't use the "cache" of previously evaluated
   % individuals)
   if params.usetestdata
       params.keepevalssize=0;
       fprintf('\n   (turning off ''keepevals'' cache)\n');
   end
