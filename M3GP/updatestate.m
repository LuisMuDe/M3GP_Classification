...
if isempty(state.bestsofar) || ((params.lowerisbetter && pop(bestindex).fitness<state.bestsofar.fitness) || (~params.lowerisbetter && pop(bestindex).fitness>state.bestsofar.fitness))
    %%LMD pruning
     if isempty(pop(bestindex).Pruned)|| strcmp(pop(bestindex).Pruned, 'False')  
       pop(bestindex).BeforePruningFitness = pop(bestindex).fitness;
       pop(bestindex).BeforePruningTree= pop(bestindex).tree;        
        
       pop(bestindex)= PrunningTheBestRandom(pop(bestindex),data,params);
       
       pop(bestindex).AfterPruningFitness =  pop(bestindex).fitness;
       pop(bestindex).AfterPruningTree = pop(bestindex).tree; 
     end
    %%%%%%%%%%%end LMD modification
...
if (strcmp(Pruning, 'Best'))
    if isempty(pop(1).Pruned)|| strcmp(pop(1).Pruned, 'False') 
       pop(1).BeforePruningFitness = pop(1).fitness;
       pop(1).BeforePruningTree= pop(1).tree;         
        
       pop(1)= PrunningTheBest(pop(1),data,params);
        
       pop(1).AfterPruningFitness =  pop(1).fitness;
       pop(1).AfterPruningTree = pop(1).tree;     
    end
elseif (strcmp(Pruning, 'BestRandom'))  
    
    try
    BestArrayPosition = find([pop.id] ==  state.bestsofar.id);
    if isempty(pop(BestArrayPosition(1)).Pruned)|| strcmp(pop(BestArrayPosition(1)).Pruned, 'False') 
       pop(BestArrayPosition(1)).BeforePruningFitness = pop(BestArrayPosition(1)).fitness;
       pop(BestArrayPosition(1)).BeforePruningTree= pop(BestArrayPosition(1)).tree;        
        
       pop(BestArrayPosition(1))= PrunningTheBestRandom(pop(BestArrayPosition(1)),data,params);
       
       pop(BestArrayPosition(1)).AfterPruningFitness =  pop(BestArrayPosition(1)).fitness;
       pop(BestArrayPosition(1)).AfterPruningTree = pop(BestArrayPosition(1)).tree;       
    end    
    ...
    for ccont = 1 : length(pop)
         if isempty(pop(ccont).Pruned)|| strcmp(pop(ccont).Pruned, 'False')    
           pop(ccont).BeforePruningFitness = pop(ccont).fitness;
           pop(ccont).BeforePruningTree= pop(ccont).tree;
             
           pop(ccont)=  PrunningTheBestRandom(pop(ccont),data,params);
             
           pop(ccont).AfterPruningFitness =  pop(ccont).fitness;
           pop(ccont).AfterPruningTree = pop(ccont).tree;               
        end        
    end
end
